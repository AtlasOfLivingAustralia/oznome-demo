...
class VisualisationController {
    ...
    def visualisation() {
        ...
        //Patents by year
        result.put("patentByYear", [["Period", "Number"]] + patentsList.countBy {
            it.filingDate[0..3].toInteger()
        }.collect { k, v -> [k, v] })

        //Patents by status
        result.put("patentsByStatus", [["Status", "Number"]] + patentsList.countBy {
            it.filingStatus
        }.collect { k, v -> [k, v] })
        ...
        def i = 0
        result.put("top20Applicants", [["Entity", "Number"]] + applicants.findAll {
            i++ < 20
        }.collect { k, v -> [k, v] })
